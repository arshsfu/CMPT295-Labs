...
void *handle_client(void *arg) {
  ...
  while ((bytes_read = read(cfd, buffer, BUF_SIZE - 1)) > 0) {
    buffer[bytes_read] = '\0';

    pthread_mutex_lock(&count_mutex);
    total_message_count++;
    printf("Msg # %3d; Client ID %d: %s", total_message_count, id, buffer);
...
int main() {
  ...
  for (;;) {
    ...
    printf("New client created! ID %d\n", client_id);
