...
void print_out(char *format, void *data, size_t data_size) {
  char buf[buff_size];
  ssize_t len =
      snprintf(buf, buff_size, format,
               data_size == sizeof(uint64_t) ? (unsigned long)*(uint64_t *)data
